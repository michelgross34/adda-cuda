...
#	endif
#elif defined(FFTW3) && !defined(ADDA_CUDA) // CPU MatVec plans
	int lot;
...
#	endif
...
#endif
#if !defined(OPENCL) && !defined(ADDA_CUDA)
	// allocate host memory for the CPU MatVec backend
	...
	fftInitAfterD();
#ifdef ADDA_CUDA
	/* D/R and geometry are now final. Upload them once and create persistent cuFFT plans. */
	CudaMatVecInit();
#endif
...
#	endif
	...
	Free_cVector(Dmatrix);
#ifdef ADDA_CUDA
	if (surface) Free_cVector(Rmatrix);
#else
